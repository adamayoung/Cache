...
final class CacheStoreTests: XCTestCase {
  ...
  override func setUp() {
    super.setUp()
    cacheStore = CacheStore(name: "cacheStoreTests")
  }
...
extension CacheStoreTests {
  ...
  func testItemForKey_whenDiskCacheContainsItemAndMemoryCacheDoesNotContainItem_returnsItem() {
    let key = UUID().uuidString
    let expectedResult = MockCacheItem(name: "some_name")
    cacheStore.setItem(expectedResult, forKey: key)

    let otherCacheStore = CacheStore(name: "cacheStoreTests")

    let result: MockCacheItem? = otherCacheStore.item(forKey: key)

    XCTAssertEqual(result, expectedResult)

    otherCacheStore.removeAll()
  }
